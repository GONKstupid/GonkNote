...
### ▶ Hier geht es weiter (Stand 2026-08-23, nach Runde V2-71)
...
**Tests laufen lassen:**
```powershell
dotnet test -c Release        # Windows: beide Projekte, 889 Tests
```

```bash
dotnet test tests/GonkNote.Core.Tests   # Linux: 830 Tests (nur Core — der WPF-Kopf baut dort nicht)
```
...
(§4.52, vorgezogen) und die FORMEN (§4.53).

Zieh zuerst den Stand: git pull. Dann bauen und testen, bevor du etwas
anfasst. Erwartet sind 0 Fehler, 0 Warnungen und 889 Tests
(830 Core + 59 WPF) -- stimmt eine der Zahlen nicht, IST DAS DER ERSTE
BEFUND.

JETZT SIND TEXT, NOTIZZETTEL UND STICKER DRAN -- Stueck 2 der
Reihenfolge in §6. Sie erben das Muster aus den Formen (anlegen,
auswaehlen, Undo, Werkzeugleiste) und BENUTZEN DEN FARBWAEHLER, DER SEIT
§4.52 STEHT (ColorPickerWindow.Waehlen). Sticker brauchen zusaetzlich die
Sammlung aus dem Datenordner.
...
### ▶ Aktueller Auftrag — **Phase 4.5: Stück 1 ist zu, als Nächstes Text/Notizzettel/Sticker** (Stand 2026-08-23, nach Runde V2-71)

> **✅ Die Formen stehen im Linux-Kopf** (§4.53, V2-71): Linie, Pfeil, Rechteck, Ellipse und
> Dreieck samt Füllung und Deckkraft. Davor die Zwischenrunde **Farbwähler** (§4.52, V2-70),
> auf Nutzer-Entscheidung vorgezogen, weil er auch Text, Notizzettel und Cover blockierte.
> **Bau 0/0 in Debug und Release, 889 Tests** (830 Core + 59 WPF).
>
> **⚠ Beide Runden haben beim Messen ihren Zuschnitt geändert — das ist inzwischen das Muster
> und keine Ausnahme:**
>
> - **§4.52:** `HexColor` in Core konnte schon Parsen **und** Formatieren; der WPF-Dialog
>   benutzte es nur nicht. Es fehlte **nur die HSV-Rechnung**.
> - **§4.53:** `WhiteboardView.Shapes.cs` (305 Zeilen) ist **nicht** das Formen-Werkzeug,
>   sondern der **Formen-Stift** plus Glättstift — rund **240 Zeilen reine Geometrie im Kopf**.
>   Das Werkzeug selbst war klein, weil `WbRenderer.DrawShape` längst `public` war.
>
> **⚠ Zwei Fehler, die nur das laufende Programm gezeigt hat, beide in §4.53:**
>
> 1. `DestinationRect="0,0,12,12px"` wirft zur Laufzeit — Avalonia liest jeden Wert als
>    `double`, ein Suffix gibt es nicht, und **der Bau war grün**, weil XAML-Attribute erst
>    beim Laden geparst werden. **Der Kommentar dazu war eine ungeprüfte Deutung**,
>    hingeschrieben statt gemessen.
> 2. `SetTool` klappte die Einstellungsleiste auf, ohne `EinstellungenSpiegeln` zu rufen — sie
>    ging mit **lauter leeren Umschaltern** auf. **Der Fehler steht seit Phase 3 wörtlich in
>    `Einstellungen_Click`**, und er ist am zweiten Aufklappweg prompt wieder eingebaut worden.
>    *Ein Warnhinweis nützt nur an der Stelle, an der jemand hinsieht.*
>
> **⚠ Eine benannte Verhaltensänderung aus §4.52:** Das Hex-Feld des WPF-Wählers nahm über
> `ColorConverter` auch Farb**namen** an („Red"). Das tut es nicht mehr — es ist ein Hex-Feld,
> und ein Extra, das nur ein Kopf kann, ist gegen M2.
>
> **✅ Beide Köpfe an derselben Datei gemessen** (Dauerregel 4): im Linux-Kopf alle fünf Formen
> gezeichnet und über den neuen Wähler gefüllt, im WPF-Kopf geöffnet — **alle sechs kommen an**,
> mit Farbe, Füllung und Deckkraft. Die fünf neuen Symbole sehen in beiden Köpfen gleich aus;
> der WPF-Kopf zeigte dort bis dahin **Unicode-Zeichen in „Segoe UI"**, die unter Linux leere
> Kästchen gewesen wären (§4.31).
>
> ---
>
> **▶ Als Nächstes: Text, Notizzettel und Sticker** — Stück 2 der Reihenfolge in §6. Sie erben
> das Muster aus den Formen (anlegen, auswählen, Undo, Werkzeugleiste) und **benutzen den
> Farbwähler, der seit §4.52 steht**. Sticker brauchen zusätzlich die Sammlung aus dem
> Datenordner.
>
> **Die Reihenfolge dahinter** (§6, unverändert): Bild-/PDF-Import → Lineal und Geodreieck →
> Zahlenblock, Schnellaktionen, Leistenordnung → OCR (braucht den Laptop). **Der Formen-Stift**
> ist seit §4.53 vermessen und gehört als eigene **Zusammenlegung** hinter Stück 1.
>
> **Die Hausregel gilt unverändert und hat sich in beiden Runden bezahlt gemacht:** eine
> Entscheidung, die auf einer **ungeprüften Deutung** steht, ist selbst ungeprüft. **Erst
> messen, dann bauen** — und *bauen* heißt auch: am laufenden Programm ansehen, denn ein
> grüner Bau und 889 grüne Wächter haben beide Fehler oben nicht bemerkt.
>
...
> | **2** | **Text, Notizzettel, Sticker** | Erben das Muster aus (1). Sticker brauchen zusätzlich die Sammlung aus dem Datenordner |
...
dotnet test -c Release       # beide Testprojekte, 889 Tests (830 Core + 59 WPF)
