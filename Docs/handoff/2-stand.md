...
| | |
|---|---|
| **Version** | **1.0.3** · `net10.0` · SkiaSharp 3.119.4 · Avalonia 12.1.1 · SQLite — 1.0.0 in V2-125, dann 1.0.1 (V2-129), 1.0.2 (V2-129b), **1.0.3 in V2-132** (eigene Designs + Stift-/Finger-Behebung). Die Stellen führt der Kommentar in `Directory.Build.props`; `About.Version` trägt keine Zahl und bleibt unverändert. **Die Über-Dialoge sind bei 1.0.3 nicht gegengeprüft** — kein Windows-Rechner, und der Linux-Über-Dialog ist nicht am laufenden Programm gesehen worden (nur das AppImage gestartet) |
