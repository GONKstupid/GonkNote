...
EINMAL FRAGEN. Daraus stehen DREI RUNDEN in dieser Reihenfolge:
 ...
 (1) ERLEDIGT am 2026-08-21 (§4.46, V2-64): TdStil rechnet in echten
     Punkten. Die Richtung ist dabei UMGEDREHT worden -- nicht der
     WPF-Kopf zeigte zu klein, sondern TdStil war die Kopie, die von
     Pixeln auf Punkt umetikettiert wurde. Nebenwirkung: die dritte
     Luecke aus §4.45 (Gliederungsebene) hat sich von selbst
     geschlossen. ES SIND NOCH ZWEI: Diagramm und Feld.

 (2) ERLEDIGT am 2026-08-22 (§4.47, V2-65): AusModell laedt direkt
     statt ueber ein XamlPackage, die Uebernahme steht als
     TdZuFlow.InhaltUebernehmen neben Umwandeln. Die Messung hat den
     Entwurf umgeworfen: WPF KOPIERT ein Tag beim Teilen eines
     Absatzes UND eines Laufs auf beide Haelften -- ein Traeger dort
     waere nach einem Tastendruck doppelt. UEBRIG BLEIBT DER
     InlineUIContainer (unteilbar). Nebenbefund: das Paket schiebt die
     Dokumentschrift als oertlichen Wert auf jeden Absatz -- derselbe
     Fehler wie §4.45; der direkte Weg raeumt ihn mit weg.
     DIE TRAEGER SELBST SIND NICHT GEBAUT -- sie gehoeren zu (3).

 (3) ERLEDIGT am 2026-08-22 (§4.48, V2-66): Rtf fuehrt nicht mehr.
     Der WPF-Editor liest UND schreibt das Modell, Migrate laeuft nur
     noch fuer die einmalige Uebernahme, TdFuehrung.AltformatFuehrt ist
...
offen ist dort nur, was mit dem Avalonia-Fund geschieht (umgehen,
melden, stehen lassen). Kurzfassung: Avalonia laesst XFilterEvent
filtern, holt das Ergebnis aber nie ab -- libX11 legt das fertige
Zeichen als Ereignis mit keycode = 0 nach, und Avalonia liest es ueber
LookupKey(keycode) statt ueber XmbLookupString aus. Der Druck kommt an,
ein Phantom gibt es nicht, unser Kopf ist es nicht. Upstream ist
...
>
> **Was diese Runde gefunden hat, und warum sie überhaupt stattfand:** Schritt 7 dreht die
> Führung auf den Weg `Modell → FlowDocument → Modell`. **Ob dieser Weg sich selbst umkehrt,
> hatte nie jemand gemessen** — er tat es nicht. Ein Modell, wie der Linux-Kopf es anlegt,
> verlor auf **einer** Rundreise Absatzabstand, Ausrichtung, Einzüge, Tabellenrahmen,
> Zellabstand, Spaltenbreite und den Abstand des Listenpunkts. **Der Kern war ein `?? 0`:**
> `TdZuFlow` hat die Kaskade aus §4.14 nicht aufgelöst, sondern durch Null ersetzt — und
> `TdParaFormat.Standard.SpaceAfterPt` ist **8**. Alles behoben, mit Wächtern belegt, und der
> wichtigste davon vergleicht Byte für Byte: **zweimal speichern ändert nichts mehr.**
>
> **⚠ Und der Verlust wartet nicht auf Schritt 7 — er läuft seit §4.23:** `Migrate` wird bei
> jedem Speichern gerufen, also verliert ein unter Linux oder aus DOCX importiertes Dokument
> **heute schon beim ersten Speichern im WPF-Editor** Tabellenrahmen und Absatzabstände (und,
...
>
> **✅ Runde ① ist erledigt** (§4.46, V2-64; Bau 0/0, **835 Tests**, in beiden Köpfen am
> laufenden Programm gesehen). **Und sie hat unterwegs die Richtung gewechselt:** Nicht der
> WPF-Kopf zeigte zu klein — **`TdStil` war die Kopie, die von Pixeln auf Punkt umetikettiert
> wurde** (§4.39). Belegt an drei Stellen, die alle schon dastanden, und am eigenen Kopf: Die
> Vorlage „Standard" machte einen unberührten Absatz um ein Drittel **größer**. `TdStil` ist
> deshalb **verkleinert** worden, der WPF-Kopf bleibt.
>
> ✅ **Nebenwirkung und bestes Ergebnis der Runde: eine der drei Lücken aus §4.45 hat sich von
> selbst geschlossen.** Die **Gliederungsebene** überlebt die Rundreise wieder — sie war keine
> Eigenschaft des `FlowDocument`, sondern derselbe Zahlendreher. **Es sind noch zwei**
> (Diagramm und Feld).
>
> **✅ Und Runde ② ist am 2026-08-22 erledigt** (§4.47, V2-65; Bau 0/0, **840 Tests**,
> 5 neue Wächter, Führung unverändert). **Die Auflage „erst messen, dann bauen" hat den
> Entwurf umgeworfen, bevor eine Zeile davon stand:** WPF **kopiert** ein `Tag` beim Teilen
> eines Absatzes **und** eines Laufs auf **beide** Hälften — ein Träger dort wäre nach einem
> Tastendruck doppelt vorhanden und damit **schlimmer als die Lücke, die er schließen
> sollte**. **Übrig bleibt der `InlineUIContainer`**: unteilbar, und derselbe Ort, an dem
> `DocumentImages` seinen Blob-Verweis seit jeher führt. **Der Ladeweg ist gebaut**, die
> Träger selbst gehören zu Schritt 7 — sie überlebten sonst das Laden und nicht das Speichern.
>
> **✅ Und Schritt 7 ist am 2026-08-22 gefahren** (§4.48, V2-66; Bau 0/0, **837 Tests**).
> **`Rtf` führt nicht mehr:** Der WPF-Editor liest und schreibt das Modell, `Migrate` läuft nur
> noch für die einmalige Übernahme, `TdFuehrung.AltformatFuehrt` ist **gelöscht**, der
> Warnstreifen im Linux-Kopf ist entfallen. **✅ §5 „Noch offen" 9 ist an der Wurzel erledigt.**
...
> ohne — **ein Träger im Arbeitsspeicher überlebt jetzt eine vollständige Runde aus Speichern
> und Öffnen.**
>
> **Und welcher Träger es sein muss, ist gemessen und nicht zu erraten** (§4.47): der
> **`InlineUIContainer`**. `Tag` an Absatz und Lauf wird von WPF beim Teilen **kopiert** — ein
> Träger dort wäre nach einem Tastendruck doppelt vorhanden, **schlimmer als die Lücke**.
...
> Beide Runden haben ihren Entwurf beim Messen verloren.
>
