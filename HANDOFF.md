...
> **▶ Stand 2026-08-23: das Schreiben ist fertig, Phase 4.5 läuft.** Schritte 1 bis 7 stehen
...
> **Formen** (§4.53). **Als Nächstes: Text, Notizzettel und Sticker** (§6, Stück 2).
...
PHASE 4.5 LAEUFT, STUECK 1 IST ZU. Zuschnitt und Reihenfolge sind seit
2026-08-22 entschieden -- NICHT NOCH EINMAL FRAGEN: volle Breite, ABER
OHNE DIE RECHTSCHREIBPRUEFUNG, Reihenfolge in §6 (Kasten "Zuschnitt und
Reihenfolge").

Erledigt sind: die Auswahl (Drehen/Skalieren, §4.51), der Farbwaehler
(§4.52, vorgezogen), die FORMEN (§4.53) und die Zusammenlegung fuer
Stueck 2 (§4.54).
...
JETZT SIND TEXT UND NOTIZZETTEL IM LINUX-KOPF DRAN -- Stueck 2, zweite
Runde. DIE ZUSAMMENLEGUNG IST SCHON ZU (§4.54): Bildsammlung,
StickerLibrary, HexColor.LesbareSchrift und HexColor.MitGenugKontrast
liegen in Core, der WPF-Kopf rechnet nichts davon mehr selbst.

Was fehlt, ist reine Kopf-Arbeit und vermessen: ein Eingabefeld ueber
der Zeichenflaeche (im WPF-Kopf eine TextBox in einem Canvas ueber dem
SKElement, WhiteboardView.xaml Zeile 565), BeginTextInput und
BeginStickyInput (Input.cs ab 370), CommitTextEdit und
CommitStickyEdit (Editing.cs), zwei Werkzeugknoepfe und zwei
Einstellungs-Sektionen. DIE SYMBOLE TextTool UND StickyNote STEHEN
BEREITS IM SATZ -- hier droht kein zweiter Fall wie bei den Formen.
WbRenderer.DrawText, DrawSticky und DrawStickyCard sind public, die
Anzeige ist also fertig. DANACH RUNDE 3: DIE STICKER.

MERKEN: leerer Text LOESCHT ein Textfeld, ein Zettel bleibt auch leer
stehen. Das ist Absicht und in Editing.cs begruendet.

ERST MESSEN, DANN BAUEN. Das ist die Hausregel, und die letzten DREI
Runden haben ihren Zuschnitt beim Messen geaendert:

 (1) §4.51: Der Undo-Weg lag FERTIG in Core, die Griff-Geometrie PRIVAT
     im WPF-Kopf. Also eine Zusammenlegung und kein Neubau.
 (2) §4.52: HexColor in Core konnte schon Parsen UND Formatieren -- der
     WPF-Dialog benutzte es nur nicht. Es fehlte NUR die HSV-Rechnung.
 (3) §4.53: WhiteboardView.Shapes.cs (305 Zeilen) ist NICHT das
     Formen-Werkzeug, sondern der FORMEN-STIFT plus Glaettstift. Das
     Werkzeug selbst war klein, weil WbRenderer.DrawShape laengst public
     war.
 (4) §4.54: Der Sticker-Nutzerordner war WINDOWS-FEST (von Hand aus
     Environment.SpecialFolder.ApplicationData), und die Endungsliste
     gehoerte zwei Werkzeugen statt einem -- DAS HAT DER UEBERSETZER
     GEMELDET, nicht das Lesen. Dazu eine zweite Kontrastrechnung, die
     beim ersten Durchgang uebersehen wurde, weil nur Editing.cs
     gelesen wurde und nicht Settings.cs.

FUER TEXT/NOTIZZETTEL/STICKER HEISST DAS: sieh zuerst nach, was in Core
schon steht (TextElement, StickyNoteElement, WbRenderer zeichnet beide)
und was im WPF-Kopf nur privat liegt -- BEVOR du im Linux-Kopf etwas
anlegst.

UND: EIN GRUENER BAU BEWEIST HIER FAST NICHTS. Die letzten zwei Runden
haben je einen Fehler gehabt, den erst das laufende Programm zeigte:

 - InputInProgress kannte einen neuen Zug nicht -> es passierte GAR
   NICHTS, bei 0 Fehlern und lauter gruenen Waechtern. WER EINEN NEUEN
   ZUG UEBER DIE FLAECHE BAUT, TRAEGT IHN DORT EIN.
 - DestinationRect="...px" warf zur LAUFZEIT -- XAML-Attribute werden
   erst beim Laden geparst. Und der Kommentar dazu war eine UNGEPRUEFTE
   DEUTUNG, hingeschrieben statt gemessen.
 - SetTool klappte die Einstellungsleiste auf, ohne
   EinstellungenSpiegeln zu rufen -> lauter leere Umschalter. Der
   Fehler stand seit Phase 3 WOERTLICH in Einstellungen_Click. Ein
   Warnhinweis nuetzt nur an der Stelle, an der jemand hinsieht.
...
WAS SONST NOCH ZU WISSEN IST:
 ...
 - Der WPF-Kopf zeigte bei den Formen Unicode-Zeichen in "Segoe UI".
   Umgestellt auf den Symbolsatz (§4.31). WER EIN WERKZEUG PORTIERT,
   PRUEFT ZUERST, OB SEIN SYMBOL UEBERHAUPT IM SATZ STEHT -- fuer Text,
   Notizzettel und Sticker steht es (TextTool, StickyNote, Sticker).
 - Das Hex-Feld des Farbwaehlers nimmt keine FarbNAMEN mehr an. Bewusst,
   wegen M2.
 - Avalonia: x:Name an einem PINSEL erzeugt kein Feld (nur Controls
   landen im Namescope), und ImageBrush.Source nimmt kein DrawingImage.
...
Lies in dieser Reihenfolge: §4.53 und §4.52 (zuletzt gefahren), §4.51
(die Auswahl und das Muster fuer Zusammenlegungen), §6 "Vorgemerkt:
Phase 4.5" samt dem Kasten "Zuschnitt und Reihenfolge", §4.28 (wie der
Linux-Kopf gebaut ist), §4.10 und §4.12 (die Zeichenflaeche) und §4.13
("pixelgleich ist hier gemessen"). Dazu §7, vor allem "Der Avalonia-Kopf"
und "Neu aus §4.51".
...
§4.51 nicht mehr. Die Abschnitte sind Chronik und nicht Vorschrift.
...
### ▶ Aktueller Auftrag — **Phase 4.5: Text und Notizzettel im Linux-Kopf** (Stand 2026-08-23, nach Runde V2-72)

> **✅ Stück 2 hat angefangen — die Zusammenlegung ist zu** (§4.54, V2-72 und V2-72a).
> Alles, was Text, Notizzettel und Sticker an geteilter Rechnung brauchen, liegt jetzt in
> Core; der WPF-Kopf rechnet nichts davon mehr selbst. **Bau 0/0 in Debug und Release,
> 913 Tests** (854 Core + 59 WPF), **24 neue Wächter**. Sichtbar hat sich nichts geändert —
> das war der Zweck.
>
> **⚠ Vier Funde, und zwei hat nicht das Lesen gemeldet:**
>
> 1. **Der Sticker-Nutzerordner war Windows-fest** — von Hand aus
>    `Environment.SpecialFolder.ApplicationData` gebaut, mitten in einer Regel für alle Köpfe.
>    `AppPaths` weiß das seit Phase 2. **`WbAidRenderer` nennt im Kommentar sogar „dasselbe
>    Muster wie bei Stickern"** — die Sticker hielten sich nicht daran.
> 2. **Die Endungsliste gehörte zwei Werkzeugen** (Sticker *und* Cover) — **gemeldet vom
>    Übersetzer beim Verschieben**, nicht beim Lesen.
> 3. Die **Zettel-Schriftfarbe** lag privat im Kopf. Sie wandert mit dem Zettel **in die
>    Datei** — zwei Fassungen gäben demselben Zettel je nach Kopf eine andere Schrift.
> 4. **Eine zweite Kontrastrechnung, in Runde 1 übersehen** (`EnsureReadableTextColor` steht
>    in `Settings.cs`, gelesen worden war nur `Editing.cs`). Nachgezogen, weil sie zur selben
>    Zusammenlegung gehört.
>
> **Die zwei Farbmethoden sind bewusst zwei:** `LesbareSchrift()` **bestimmt** eine
> Schriftfarbe (der Zettel hat keine gewählte), `MitGenugKontrast()` **überstimmt** eine
> gewählte nur, wenn sie unlesbar wäre (ein Textfeld erbt die Tintenfarbe des Nutzers, und die
> soll er behalten, solange man sie sieht).
>
> ---
>
> **▶ Als Nächstes: Text und Notizzettel im Linux-Kopf.** Beide zusammen, denn sie brauchen
> **dieselbe Naht** — ein Eingabefeld über der Zeichenfläche. Was zu tun ist, ist vermessen:
>
> | | |
> |---|---|
> | **Die Naht** | Im WPF-Kopf eine `TextBox` in einem `Canvas` über dem `SKElement`, positioniert über `ToScreen` (`WhiteboardView.xaml`, Zeile 565). Der Linux-Kopf braucht das Gegenstück über seinem `SkiaCanvas` |
> | **Anlegen** | `BeginTextInput`/`BeginStickyInput`: vorhandenes Element unter dem Zeiger treffen → bearbeiten, sonst neu anlegen (`WhiteboardView.Input.cs`, ab Zeile 370) |
> | **Ablegen** | `CommitTextEdit`/`CommitStickyEdit` in `WhiteboardView.Editing.cs` — **leerer Text löscht ein Textfeld, ein Zettel bleibt auch leer stehen**; das ist Absicht und steht dort begründet |
> | **Werkzeuge** | Zwei Knöpfe. **Die Symbole `TextTool` und `StickyNote` stehen bereits im Satz** — hier droht kein zweiter Fall wie bei den Formen (§4.53) |
> | **Einstellungen** | Zwei Sektionen: Textfarbe/Hintergrund und Zettelfarbe, beide über den Farbwähler aus §4.52 |
>
> **Steht schon:** `WbRenderer.DrawText`, `DrawSticky` und `DrawStickyCard` sind `public` — die
> Anzeige ist fertig. Danach **Stück 2, Runde 3: die Sticker** (`StickerLibrary` liegt bereit).
>
> **⚠ Die Warnung aus §4.53 gilt für die neue Naht unmittelbar:** Ein grüner Bau beweist hier
> fast nichts. Zweimal in Folge hat erst das laufende Programm den Fehler gezeigt — einmal ein
> vergessener Eintrag in `InputInProgress` (es passierte **gar nichts**), einmal ein
> XAML-Attribut, das erst beim Laden geparst wird. **Erst messen, dann bauen — und bauen heißt
> auch: ansehen.**
>
...
> | **2** | **Text, Notizzettel, Sticker** — ⏳ **die Zusammenlegung ist zu** (§4.54, V2-72): `Bildsammlung`, `StickerLibrary`, `HexColor.LesbareSchrift`/`MitGenugKontrast` liegen in Core. Es fehlt die Bedienung im Linux-Kopf | Erben das Muster aus (1). Sticker brauchen zusätzlich die Sammlung aus dem Datenordner |
