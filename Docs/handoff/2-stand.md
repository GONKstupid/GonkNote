...
| | |
|---|---|
| **Version** | **1.0.4** · `net10.0` · SkiaSharp 3.119.4 · Avalonia 12.1.1 · SQLite — 1.0.0 in V2-125, dann 1.0.1 (V2-129), 1.0.2 (V2-129b), 1.0.3 (V2-132), **1.0.4 in V2-135/V2-136** (Rechtschreib- und Grammatikprüfung unter Linux + Schriftvorschau). Die Stellen führt der Kommentar in `Directory.Build.props`; **`About.Version` trägt diesmal mit** — der Satz dahinter nannte die Rechtschreibprüfung als Nächstes und tut es nicht mehr. **Die Ausgabe ist auf Wunsch des Nutzers zurückgehalten worden, bis die Grammatik drin war** (2026-09-16). **Die Über-Dialoge sind bei 1.0.4 nicht gegengeprüft.** |
...
| **5.2** — Grammatikprüfung im Linux-Kopf | ✅ **abgeschlossen 2026-09-16 (V2-136)**, geht mit 1.0.4 hinaus — feste Regeln in Core (`TdGrammatik`), die immer laufen, und `TdLanguageTool` darüber, **wenn auf dem Rechner ein Server läuft**; zusammengelegt in `TdPruefung`. ⚠ **Stufe 1 ist keine Grammatikprüfung im Sinne von LanguageTool** — ohne Wortarterkennung sind Kongruenz, Fälle und Satzbau nicht prüfbar, und das steht im Quelltext ganz oben. **Nur Loopback**, die öffentliche API ist bewusst nicht unterstützt. | §6 |
